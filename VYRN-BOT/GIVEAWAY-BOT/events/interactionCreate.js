...
const { handleExpeditionSelect } = require("../utils/expeditionPanel"); 
  ...
  async execute(interaction, client) {
    try {
      ...
      if (interaction.isButton() && interaction.customId === "daily_claim") {
        ...
        if (!isDailyReady(userId)) return interaction.reply({ content: "❌ Daily not ready yet!", ephemeral: true });
        const reward = await claimDaily(userId, interaction.member);
        if (!reward || reward.error) return interaction.reply({ content: "❌ Cannot claim daily yet!", ephemeral: true });
