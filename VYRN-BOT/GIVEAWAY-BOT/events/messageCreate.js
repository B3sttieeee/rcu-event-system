...
// Poprawne importy
const { addXP, loadConfig } = require("../utils/levelSystem");        // ← DODANE loadConfig
...
async function handleXPSystem(message) {
  ...
  const cfg = loadConfig();   // ← Teraz działa
...
async function showRank(message) {
  ...
  const cfg = loadConfig();   // ← też działa
