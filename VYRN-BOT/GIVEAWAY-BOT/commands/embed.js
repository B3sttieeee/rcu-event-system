  ...
  modal.addComponents(
    ...
    new ActionRowBuilder().addComponents(
      createInput({
        ...
        placeholder: "Opis (wpisz . aby zostawić pusty)",
        maxLength: 4000
      })
    ),
    new ActionRowBuilder().addComponents(
    ...
    new ActionRowBuilder().addComponents(
      createInput({
        ...
        style: TextInputStyle.Short,
        value: existingEmbed?.author 
          ? `${existingEmbed.author.name || ""} | ${existingEmbed.author.iconURL || ""}` 
          : "",
  ...
  async handleModal(interaction) {
    ...
    try {
      ...
      await interaction.editReply({
        content: `✅ Embed został wysłany na kanał **${channel.name}**`,
        components: [row]
      });
