...
function loadProfile() {
  ...
  try {
    ...
    console.error("[PROFILE] Błąd odczytu → tworzę nowy plik");
    dbCache = { users: {} };
    fs.writeFileSync(PROFILE_PATH, JSON.stringify(dbCache, null, 2));
  }
  return dbCache;
...
function saveProfile() {
  ...
  try {
    ...
    console.error("[PROFILE] Błąd zapisu:", err.message);
  }
}
...
function ensureUser(userId) {
  ...
  if (!data.users[userId]) {
    data.users[userId] = {
      voice: 0,
      daily: { msgs: 0, vc: 0, streak: 0, lastClaim: 0 }
    };
  }
...
function isDailyReady(userId) {
  ...
  return vcMinutes >= tier.vcRequired &&
...
async function claimDaily(userId, member = null) {
  ...
  if (now - user.daily.lastClaim < 86400000) return { success: false, error: "cooldown" };
  ...
  if (member && !member.user.bot) {
    try {
      ...
      console.error("❌ Błąd dodawania XP przy daily:", err.message);
    }
  }
