...
const { tryGiveRandomBoost, getCurrentBoost } = require("./boostSystem");
...
function loadConfig() {
  ...
  if (!configCache) {
    try {
      ...
      console.error("[LEVEL] Błąd odczytu levelConfig.json");
...
async function addXP(member, baseAmount, messageLength = 0) {
  ...
  // Losowy Lucky Boost (szansa przy każdej wiadomości)
  tryGiveRandomBoost(member);
  ...
  for (const [levelStr, roleId] of Object.entries(LEVEL_ROLES)) {
    ...
    if (currentLevel >= requiredLevel && !member.roles.cache.has(roleId)) {
      ...
      await member.roles.add(roleId).catch(err => {
        console.error(`❌ Nie dodano roli level ${requiredLevel}:`, err.message);
      });
    }
  }
}
