...
async function createTicketPanel(client) {
  ...
  if (!channel) return console.warn("[TICKETS] Panel channel not found");
  ...
  const embed = new EmbedBuilder()
    ...
    .setDescription(
      ...
      `━━━━━━━━━━━━━━━━━━\n\n` +
      `Select an option below and fill the form.`
    )
  ...
  const menu = new ActionRowBuilder().addComponents(
    new StringSelectMenuBuilder()
      ...
      .addOptions([
        { label: "VYRN Main Clan", description: "Competitive / High-tier players", value: "vyrn", emoji: "🔥" },
        { label: "Staff Support", description: "Join the moderation team", value: "staff", emoji: "🛠️" }
      ])
  );
  ...
  console.log("🎟 Ticket panel został utworzony/zaktualizowany");
}
...
async function createTicket(interaction, type, client) {
  ...
  const names = { vyrn: "VYRN Main Clan", staff: "Staff Support" };
  ...
  const channel = await interaction.guild.channels.create({
    ...
    permissionOverwrites: [
      ...
      { id: CONFIG.ADMIN_ROLE, allow: [PermissionsBitField.Flags.ViewChannel, PermissionsBitField.Flags.SendMessages, PermissionsBitField.Flags.ManageMessages] }
    ]
  });
  ...
  await interaction.editReply({ content: `✅ Ticket został utworzony: ${channel}` });
...
function init(client) {
  console.log("🎟 Ticket System → załadowany");
  // Panel jest tworzony w ready.js lub ręcznie
}
