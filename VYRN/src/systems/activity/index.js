// src/systems/activity.js
const fs = require("fs");
const path = require("path");
const { EmbedBuilder } = require("discord.js");
const economy = require("./economy"); // Zakładam że economy jest w tym samym folderze
...
function loadAll() {
  try {
    ...
    console.log("[ACTIVITY] 📊 Baza danych załadowana");
  } catch (e) {
...
function addActivityXP(member, xpAmount = 10, coinsAmount = 8) {
  ...
  // Automatyczne dodanie monet do ekonomii
  if (coinsAmount > 0) {
    economy.addCoins(member.id, coinsAmount);
  }
...
function init() {
  loadAll();
  setInterval(saveAll, 30000); // Zapis co 30 sekund
}
...
module.exports = {
  ...
  getLevelData: (userId) => {
    const user = ensureUser(userId);
    const currentLevel = user.level.level || 0;
    return {
      xp: user.level.xp || 0,
      level: currentLevel,
      totalXP: user.level.totalXP || 0,
      nextXP: neededXP(currentLevel) // Zwraca poprawne brakujące XP dla komendy /profile!
    };
  }
};
